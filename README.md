...
分析結果から、コピーではないオリジナル企画10本、最優先台本、映像・音声仕様、反応予測、検証計画を含む「再現仕様書」を作成できます。歌・音楽が主役の投稿では、オリジナル楽曲と縦型ミュージック動画の制作仕様も作成できます。すべてのアカウントで、歌を主役にするべきか、補助にするべきか、使わないべきかを先に判定します。
...
- アカウント全体の根拠から、`music-led`（歌主役）・`music-assisted`（音楽補助）・`voice-led`（話す音声主役）を判定する
...
この機能は完成MP3やMP4を自動出力するものではありません。アカウント分析の結論に応じて、音楽生成サービスへ渡せる制作指示と、制作・投稿時の権利確認を出力する機能です。音楽が有効と判断できないアカウントには、無理に歌を作成しません。
...
10. アカウントごとの音声戦略判定：歌主役／音楽補助／話す音声主役
11. 音楽主役投稿の場合：オリジナル楽曲・日本語合成音声・歌詞表示・映像同期・権利確認仕様
12. 再生、いいね、コメント、フォローの条件付き予測
13. 失敗要因、検証計画、次に必要なデータ
